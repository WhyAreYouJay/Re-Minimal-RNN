...
class minGRU(Module):
    ...
    def forward(self, x:torch.Tensor, h0=None):
        ...
        if self.down_projection is not None:
            h =  self.down_projection(parallel_scan_log(log_coeffs, torch.cat([self.log_h[:log_tilde_h.size(0)],log_tilde_h + log_z], dim=1)))
        else:
            h =  parallel_scan_log(log_coeffs, torch.cat([self.log_h[:log_tilde_h.size(0)],log_tilde_h + log_z], dim=1))
