...
class minGRU_Reinformer(nn.Module):
    def __init__(
        ...
        self.embed_h = nn.Linear(self.h_dim, int(self.h_dim * expansion_factor), device=device)
        self.lt = nn.Parameter(torch.zeros((n_layers ,max_timestep, 1, int(self.h_dim * expansion_factor)), device = device))
    ...
    def forward(
        ...
        h_par = g(self.lt[:,timesteps[:,1]])
        h_par[:,timesteps[:,:1] == 0] = torch.ones_like(h_par[0,0], device=h_par.device)*0.5
